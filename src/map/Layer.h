...
template <typename PtrType> class Layer {
  int m_width{};
  int m_height{};
...
Layer<PtrType>::Layer(int width, int height)
...
template <typename PtrType> PtrType &Layer<PtrType>::operator[](int x, int y) {
  return m_grid[static_cast<std::size_t>(y)][static_cast<std::size_t>(x)];
}

template <typename PtrType> int Layer<PtrType>::getWidth() const {
  return m_width;
}

template <typename PtrType> int Layer<PtrType>::getHeight() const {
  return m_height;
}