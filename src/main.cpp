...
GameSession &prepGame(GameSession &gameSession) {
  ...
  Container cont(std::move(containerItems), Point(0, 0), "placeholder", "chest",
  ...
  auto npc{std::make_shared<NonPlayableCharacter>(
      'e', Point(4, 7), "placeholder", 12,
      std::vector<std::shared_ptr<Item>>{enemyLoot}, "Gruff-looking man",
  ...
  auto npc2{std::make_shared<NonPlayableCharacter>(
      'E', Point(8, 2), "placeholder", 8,
      std::vector<std::shared_ptr<Item>>{enemyLoot2}, "Strange-looking man",
...
int main() {
  GameSession gameSession{
      20, 10, std::make_shared<Player>(Point(2, 1), "placeHolder", 10)};
  std::unordered_map<std::string, std::shared_ptr<Item>> items{
      DataLoader::getAllItems()};
  std::cout << *(items["itemBanana"]);
  std::unordered_map<std::string, Map> m{
      DataLoader::getAllMaps(items, gameSession)};
  std::cout << m.at("level1");
}

// int main() {
//   GameSession gameSession{20, 10, std::make_shared<Player>(Point(2, 1),
//   10)}; prepGame(gameSession);
...
//         Command::command
//         command{CommandHandler::getCommand(Input::getKey())};
//         CommandHandler::executeWorldCommand(gameSession, command);
//       }
